...
var firstMissingPositive = function(nums) {
    for(let i = 0;i<nums.length;i++){
        // 原位hash，把n放在numbers[n-1]的位置上。
        // 假如1-N(这里的N指的是nums.length)个数组是从1开始的连续数字，那么最小值就是N+1
        // 否则，最小值一定是在[1,N]中产生
        // 所以只要判断1-N这些数字中哪个数字有缺漏，那他就是最小的数字
        // nums[num[i]-1] !== nums[i]用来做数字的交换，举个例子
        // [1, 3, 4000,1] -> [1, 4000, 3, 1] 这时候4000的位置就被前置了，需要继续进行交换
        // 所以需要用到while，但是4000是在范围外，所以就要跳出循环进行处理
...
这个方法在leecode上需要做还原链表的操作，面试的时候应该就不需要。
...
先用obeject记录次数，然后格式化成entries，进行排序，返回前k个key值。
...
visitedCharMap：记录当前遍历到的字母有没有被使用过。

result：记录分割片段的长度

strMapArr：记录分割片段的字母map，与result一一对应。

一个result，一个result记录分割片段的map。

- 若当前字母不存在于visitedCharMap，那么result可以推入一个新的分割片段，
- 若当前字母存在于visitedCharMap，那么就遍历strMapArr，找到存在当前字母的那个片段，往后进行合并。

思路2：

charLastIdxMap：单词最后一次出现的下标。

lastIndex：分割片段中最远的单词坐标

_lastIndex：每次遍历的产物，如果比lastIndex大，则更新lastIndex

先遍历字符串，构建charLastIdxMap。

接着再次遍历字符串

- 如果i === lastIndex，则到达了最远单词坐标，记为一个独立片段
- 否则视情况更新lastIndex
...
- 遍历字符串，当遍历到单词i的时候，字母被拆分为前后两部分
  - dp[i]  = false，说明前半部分不被包含
  - dp[i] = true，说明单词的前半部分被包含
    - 那就检测单词列表是否包含字符串的后半部分
      ```js
