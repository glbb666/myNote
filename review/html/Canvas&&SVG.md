...
用`canvas`进行压缩的时候，图像的`dpi`不会发生变化，在把`canvas`转换为`Blob`的时候，可以设置第三个参数，这个参数可以改变`dpi`的大小。
...
`canvas`压缩图片，假如把`10000 * 10000`的图片压缩成`120 * 120`的，图片的`dpi`是不会发生变化的，但是比较图片是否清晰，是建立在图片大小相等的基础上进行比较，如果我需要的是`200 * 200`的图片，那我这个`120 * 120`的图片就肯定需要进行放大，这时就会出现模糊的问题。所以我们解决的方案是，让用户自己选择压缩图片的大小，这样可以确保缩小的图片不因为放大出现模糊的问题，用户还可以传入`quality`参数设定`dpr`（`quality`参数的默认值是`0.92`），这样就可以根据自己的要求获得清晰或者模糊的图片了。

### Canvas适配高倍屏

因为`canvas`绘制的是位图，所以在高倍屏，比如(`dpr=2`)，一个逻辑像素由4个物理像素组成，一个位图像素要填充4个物理像素，因为位图像素不可分割，所以只能就近取值，造成图片模糊。我们需要将**canvas放大到设备像素比来绘制，最后将canvas压缩成一倍的物理大小来显示。**

将`canvas`放在高倍屏绘制，最后将`canvas`压缩成一倍的物理大小来显示

用`canvas.style.width`和`canvas.style.height`设置画布外面的容器大小

用`canvas.width`和`canvas.height`设置`canvas`的画布大小
