...
`JS`分为同步任务和异步任务，同步任务都在主线程上执行，形成一个**执行栈**，主线程之外，还有两种任务队列，一种是**事件触发线程**管理的**宏任务队列**，一种是微任务队列，我看过有说法说它是**`JS`引擎线程管理的**。**宏任务队列可以有多个，微任务队列只能有一个。**只要异步任务有了运行结果，事件触发进程就会在宏任务队列之中放置一个任务。一旦**执行栈**中的所有同步任务执行完毕（此时`JS`引擎空闲），系统就会读取**宏任务队列**，将可运行的异步任务添加到可执行栈中，开始执行。
...
每次执行栈执行的代码就是一个宏任务（包括每次从任务队列中获取一个事件回调并放到执行栈中执行）
...
- 渲染完毕后，`JS`线程继续接管，开始下一个宏任务（从任务队列中获取）
