...
Q2：Context是有代价的，在什么场景下用Context

不频繁更新且深层嵌套的数据

- **避免过度的全局状态管理** ：将不需要频繁更新的数据放在 Redux 的 store 中可能导致不必要的复杂性，因为Redux是为了管理全局应用状态和响应用户交互而设计的。对于“只读”和初始化时加载的数据，使用 Context 可以减少Redux状态树的膨胀。
- **简化组件嵌套传递** ：对于需要在嵌套组件中访问的数据，如果你将它们存储在 Redux 中，你可能需要在很多层级中传递这些数据，或者使用 `connect` / `useSelector` 连接许多组件到 Redux，这可能导致不必要的渲染和性能下降。Context API可以避免这种情况，因为Context的Provider可以在组件树的任何层级提供数据，而下级组件可以直接通过 `useContext` 访问这些数据。
- **性能考量** ：由于这些数据不会改变，所以使用 Context 不会有因数据变化而导致的重渲染的性能问题。只要正确地使用 `React.memo` 或 `shouldComponentUpdate`，就可以确保组件只在必要时更新。

一些实例：

1. 深层埋点：from参数，需要埋点埋的比较深，一层层传又会让组件不纯粹。
2. **主题切换** ：
