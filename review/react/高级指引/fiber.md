...
Fiber 节点存储了组件的状态、属性、类型以及对应的 DOM 节点等信息。并且Fiber还有一个更新队列，它是一个链表，它存储着待处理的状态更新。
...
- 更新之后的Fiber状态会用来生成新的虚拟DOM
- React 使用 Diff 算法比较新旧虚拟 DOM，确定哪些部分需要实际的 DOM 更新。
- Diff 算法会遍历新旧虚拟 DOM 树，找出差异，并生成一组 DOM 更新操作。
- 最终的 DOM 更新会应用到实际的页面上。
