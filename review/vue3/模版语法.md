...
双大括号会将数据解释为纯文本，而不是 HTML。若想插入 HTML，你需要使用 [`v-html` ](https://cn.vuejs.org/api/built-in-directives.html#v-html)，详细见基础/指令v-html

## Attribute 绑定

双大括号不能在 HTML attributes 中使用。想要响应式地绑定一个 attribute，应该使用 [`v-bind`](https://cn.vuejs.org/api/built-in-directives.html#v-bind)，详细见基础/指令v-bind
