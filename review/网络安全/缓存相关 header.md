...
- 资源打开但没有修改也会更新`last-modified`的时间，服务器会因为时间匹配不上重新返回资源主体。
...
`Etag`响应头，是资源的特定版本的标识符。资源内容不改变，`Etag`不变。使用`ETag`有助于防止资源的同时更新相互覆盖（“空中碰撞”）。
...
- "<etag_value>"（无论内容发生多么细微的改变都会改变其值）

  实体标签唯一地表示所请求的资源。 它们是位于双引号之间的ASCII字符串（如“675af34563dc-tr34”）。通常，把内容通过散列算法得到它们，最后修改时间戳的哈希值，或简单地使用版本号。不。例如，MDN使用wiki内容的十六进制数字的哈希值。
