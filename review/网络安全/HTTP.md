...
- 短连接：每当客户端发送一个请求时，通常会先建立一个 TCP 连接。服务器处理请求并发送响应后，这个 TCP 连接就会立即关闭。

* `keep-alive` 头部：一些 HTTP/1.0 支持非标准的 `Connection:keep-alive`启用长连接。
...
- 长连接：HTTP/1.1支持长连接（默认情况下），在一个TCP连接上可以传送多个HTTP请求和响应。但是同一时刻只能有一个 `http`请求。
- 缓存优化：HTTP/1.1引入了更复杂的缓存控制机制，比如 `ETag`、`If-Modified-Since`。这些改进使得缓存更有效，减少不必要的数据传输。
...
- 多路复用
  **每个域名的请求在一个TCP连接中**，互不干扰。
  ...
  - HTTP2.0引入了"流"的概念，一次完整的请求-响应过程称为一个流。流是为了多路复用产生的，1.0的一次性连接和1.1的顺序连接不需要流对请求-响应的模式进行标记。
...
- 流量控制和优先级
  ...
  HTTP/2.0 允许客户端和服务器为不同的请求设置优先级，并对流量进行更精细的控制，从而优化资源分配。

## `HTTP 3.0` 优势

**基于 QUIC** ：

HTTP/3.0 基于 QUIC 协议，它使用 UDP 而不是 TCP，从而避免了 TCP 中的连接建立延迟和队头阻塞问题。

**连接迁移** ：

QUIC 支持连接迁移，即使客户端的 IP 地址发生变化（例如从 Wi-Fi 切换到移动网络），连接也能保持不中断。

**内置加密** ：

QUIC 协议自带加密，所有传输内容默认使用 TLS 1.3 加密，提升了安全性。

**更快的握手速度** ：

QUIC 通过减少握手所需的往返次数，加快了连接建立的速度，尤其适用于移动网络和高延迟网络环境。

**改进的流量控制** ：

QUIC 在传输层实现了更加灵活的流量控制，进一步优化了数据传输效率。
