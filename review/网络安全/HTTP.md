...
`https`不是一种新的协议，而是`http`+`ssl`或者`http`+`tls`。`http`用的是明文传输，这样它传输的数据就十分的不安全，于是就有了`ssl`，用于对`http`协议传输的数据进行加密。`ssl`使用了对称加密，非对称加密，和`hash`算法。`https`在传输数据前需要客户端和服务器进行一次握手，确定双发加密传输数据的密码信息。对称算法的效率高，但是因为客户端和网站用的是同一密钥，如果密钥被窃听者获取了就不够安全。非对称算法安全性够，但是效率低，为了兼顾安全和效率，我们需要用非对称加密发送密码，用密码加密数据。因此握手过程为：浏览器给网站发加密规则，网站给浏览器回证书，证书里包含了签发者，证书用途，证书到期时间，使用的`HASH`算法，签名使用的算法等，接着会把证书内容求一个`HASH`值，再用`CA`私钥把证书内容的`HASH`及其使用的`HASH`算法进行加密，完成数字签名，浏览器验证证书的合法性。

1）校验证书中包含的域名是否和正在访问的网站一致，证书是否过期

2）把操作系统内置`CA`和证书颁发者`CA`进行比对，验证合法性

3）从浏览器用`CA`公钥，对证书末尾的数字签名进行解密，取出`hash`算法，浏览器只需要用`hash`算法对证书内容进行加密，校验加密后的内容和解密后的`hash`内容是否一致，如果一致，认证通过，可以取得服务器的公钥。

如果一致，客户端就生成密码信息，用服务器的公钥进行加密，接着使用约定好的`hash`计算握手信息，并用密码对信息进行加密，再发给网站。网站用私钥将解密密码，再用密码解密浏览器发来的信息，并且验证`hash`是否和浏览器发来的一致。接着网站用密码加密一段握手信息发送给浏览器。浏览器解密并计算握手的`hash`是否和服务器发来的`hash`一致，如果一致，此时握手过程就结束，之后所有的通信数据都将由浏览器生成的随机密码利用对称加密算法进行加密。如果在握手过程中出现任何错误，都会使加密连接断开。
