`http`协议是无状态的，我们可以用`cookie`来进行一个状态的标识和追踪，当用户登陆后，服务器可以在响应头设置一个`setCookie`的字段，这样客户端在之后的每一次请求都会带上这个`cookie`，但是因为`cookie`存在客户端，所以别人可以获取到里面的内容，并不会很安全，并不能用它存储很重要的信息，所以我们就有了`session`，`session`是存在服务器端的，我们可以把重要的信息存储在服务器。`session`需要依赖`cookie`实现的，用户登陆成功后，服务器会生成一个`session`，并且把这个`session`的唯一标识，`session_id`存在`cookie`里，但是`session`存在一些问题，首先，`session`需要存储所有用户的`session_id`，这个负担是很重的，其次，`session`的扩展性不好，如果两台机器形成了集群，用户第一次通过机器`A`登陆系统，用户的`session_id`存储在`A`上，第二次请求被转发到机器`B`，就只能进行`session`复制，把`session_id`在机器`A`和机器`B`之间搬来搬去，有人说，可以把`session_id`集中存储在一个地方，所有的机器都来访问这个地方的数据，但是这样的话，如果负责存储`session_id`的机器挂了，所有的用户都得重新登录，最后，因为`session_id`还是存在`cookie`里的，所以说`session`还是会遭到我们`xxs`攻击，攻击者通过插入`js`脚本，脚本内部可以通过`document.cookie`来获取`cookie`内部的`session_id`，冒充用户身份。以及`csrf`攻击，用户登录一个A网站后未登出，服务器端就在浏览器上对这个网页`setCookie`，此时用户再登陆危险网站B，B网站中对A网站进行访问，就会造成攻击。为了预防`csrf`攻击，以及不给服务器造成那么大的存储负担。为了防止`csrf`攻击，`token`出现了，`token`是无状态的，当用户登陆的时候，服务端会把用户名和密码进行加密，生成一个token令牌，保存在用户客户端的`localStorage`中，用户每次请求可以以参数的形式加入`token`，`token`不会被请求自动携带，也不可伪造，而且存储在客户端，不会给服务器带来负担。
