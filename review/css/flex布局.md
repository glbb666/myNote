...
> 主轴有四个方向，默认方向为向右。交叉轴方向为主轴逆时针方向90°。具体看下面：轴。

### 1. 容器

> 容器具有这样的特点：父容器可以统一设置子容器的排列方式，子容器也可以单独设置自身的排列方式，如果两者同时设置，子容器优先
...
`justify-content` 属性于定义如何沿着主轴方向排列子容器。
...
![1775639558259](image/flex布局/1775639558259.png)

> **flex-end**：末尾段对齐

![1775639566453](image/flex布局/1775639566453.png)

> **center**：居中对齐

![1775639583112](image/flex布局/1775639583112.png)

> **baseline**：基线对齐，这里的 `baseline` 默认是指**首行文字**，即 `first baseline`，所有子容器向基线对齐，交叉轴起点到元素基线距离最大的子容器将会与交叉轴起始端相切以确定基线。
...
- `initial(flex: 0 1 auto)`元素不会伸长会缩短，这个是默认值。（这里的三个值按照下面的三值定义）
...
🌟注意：当使用一个或两个无单位数时, flex-basis会从auto变为0
...
- 设置基准大小：**flex-basis**
  ...
  `flex-basis` 表示在不伸缩的情况下子容器的原始尺寸。**主轴为横向时为宽度，主轴为纵向时为高度**。
...
![1775633601083](image/flex布局/1775633601083.png)

- 设置收缩比例：**flex-shrink**

  子容器弹性收缩的比例。如图，超出的部分按 1:2 的比例从给子容器中减去。

![1775633590522](image/flex布局/1775633590522.png)

- 设置排列顺序：**order**

  改变子容器的排列顺序，覆盖 HTML 代码中的顺序，默认值为 0，可以为负值，数值越小排列越靠前。

![1775633577659](image/flex布局/1775633577659.png)

---

  以上就是 flex 布局的全部属性，一共 12 个，父容器、子容器各 6 个，可以随时通过下图进行回顾。

![1775633557271](image/flex布局/1775633557271.png)
...
flex-shink属性主要是在外层div宽度不够的情况下，子div收缩一定的空间来抵消不够的那部分宽度。 举个栗子，现在三个子div的宽度是600px, 但是我给外层div的宽度设置成500px, 那么应该怎么让子div们收缩掉这100px。这个时候flex-shrink就派上用场了，那具体怎么计算呢？ 
...
### 4.比较一下 `align-items`、`align-content`、`align-self`
